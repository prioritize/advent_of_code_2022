...
impl DayEight {
   fn open_input_file(filename: String) -> Vec<Vec<u32>> {
       ...
       let tree_height = string_buf.lines().map(|x| {
           x.chars().map(|v| {
               v.to_digit(10).unwrap()
           }).collect()
       }).collect();
       tree_height
   }
...
mod tests {
    ...
    fn test_open_input() {
        ...
        let mut visible: Vec<Vec<bool>> = trees.iter().map(|x| {
            x.iter().map(|v| {
                false
            }).collect()
        }).collect();
        ...
        let true_lines: Vec<Vec<bool>> = visible.iter().map(|b| {
            b.iter().filter(|x| {
                x == &&true
            }).cloned().collect()
        }).collect();
        let visible_count: Vec<u32> = true_lines.iter().map(|i| {
            i.iter().fold(0, |acc, _| acc + 1)
        }).collect();

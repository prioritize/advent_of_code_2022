...
impl DayThree {
    fn parse(filename: String) {
        ...
        let duplicates: Vec<char> = f_string.lines().map(|x| {
            let center = x.len()/2;
        ...
        let priority_sum = duplicates.iter().map(|x| {
            p.get(x).unwrap()
        }).fold(0, |acc, v| {
            acc + v
        });
        println!("The sum of the priorities is {}", priority_sum);
        let maps: Vec<HashMap::<char, u32>> = f_string.lines().map(|x| {
            let mut hm = HashMap::new();
            x.chars().for_each(|v| {
                match hm.get(&v) {
                    None => {hm.insert(v, 1);},
        ...
        let chars: Vec<char> = groups.iter().map(|x| {
           find_shared_character(&x[0], &x[1], &x[2])
        }).collect();
        let badge_total = chars.iter().map(|x| {
            p.get(x).unwrap()
        }).fold(0, |acc, v| {acc + v});
...
fn find_shared_character(first: &HashMap::<char, u32>, second: &HashMap::<char, u32>, third: &HashMap::<char, u32>) -> char {
    let answer: Vec<Option<&char>> = first.iter().map(|(k, v)| {
        match second.get(k) {
            Some(_) => {
                 match third.get(k) {
                     Some(_) => {
...
#[cfg(test)]
mod tests {
    use crate::days::day_3::{DayThree, make_priority_map};
